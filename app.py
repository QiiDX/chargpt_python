...
api_key = "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"
...
def keyGPT():
    ...
    def generate():
        ...
        app.logger.debug("===========================")
        app.logger.debug("time: %s", time.strftime("%Y-%m-%d_%H:%M:%S"))
        app.logger.debug("userIn: %s", inputText)
        app.logger.debug("chatgpt: %s", full_reply_content)
        app.logger.debug("===========================")
    ...
    if len(messages) < 10:
        try:
            ...
            if hasattr(e, "status_code") and e.status_code == 500:
                return Response(str(e), mimetype='text/event-stream',
                                headers={'Cache-Control': 'no-cache', 'Access-Control-Allow-Origin': '*'})
            elif Response("You exceeded your current quota, please check your plan and billing details.",
                          mimetype='text/event-stream',
                          headers={
                              'Cache-Control': 'no-cache',
                              'Access-Control-Allow-Origin': '*'}):
                return Response("您的apikey配额不足，请检查登录openAI检查配额", mimetype='text/event-stream',
                                headers={'Cache-Control': 'no-cache', 'Access-Control-Allow-Origin': '*'})
    else:
